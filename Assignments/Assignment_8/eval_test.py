...
def predict_test_set(path_to_test_set, model_path):
    ...
    test_dataloader = get_dataloader(path_to_test_set, IMSIZE, BATCH_SIZE, NUM_WORKERS, anchor_grid, is_test=True,
                                     apply_transforms_on_init=True)
    model = MmpNet(len(WIDTHS), len(ASPECT_RATIOS), IMSIZE, SCALE_FACTOR).to(DEVICE)

    model.load_state_dict(torch.load(model_path))
    evaluate_test(model, test_dataloader, DEVICE, anchor_grid, f'test_output.txt', nms_threshold=NSM_THRESHOLD,
...
def main():
    ...
    path_to_test_set = './dataset_mmp/test/'
    predict_test_set(path_to_test_set, f'{run_dir}/best_model.pth')
